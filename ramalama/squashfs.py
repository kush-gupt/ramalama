...
def create_squashfs(source_model, target_path, args):
    ...
    with tempfile.TemporaryDirectory(prefix="ramalama_squashfs_") as staging_dir:
        ...
        conman_args = [
            ...
            "--network=none", f"--pull={args.pull}",
            "-v", f"{staging_dir}:/input:ro",
            "-v", f"{blobs_dir}:/blobs:ro",
            "-v", f"{target_dir}:/output:rw",
            "--env", "SOURCE_DATE_EPOCH=0",
            args.image,
            "mksquashfs", "/input", "/blobs", f"/output/{target_filename}",
            "-comp", args.compression,
            "-no-xattrs", "-noappend", "-no-exports", "-no-progress",
        ]
